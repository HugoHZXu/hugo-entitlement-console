...
const allocatedUserStatusMessageKeys: Record<AllocatedUserStatus, string> = {
...
export function formatAllocatedUserStatus(status: AllocatedUserStatus): string {
  const labels: Record<AllocatedUserStatus, string> = {
    active: 'Active',
    pending: 'Pending',
    revoked: 'Revoked',
  };
...
export function getAllocatedUserStatusMessageKey(status: AllocatedUserStatus): string {
  return allocatedUserStatusMessageKeys[status];
}
