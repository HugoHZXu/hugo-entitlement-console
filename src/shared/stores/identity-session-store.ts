...
export const useIdentitySessionStore = defineStore('identity-session', {
  ...
  actions: {
    applySession(session: DemoIdentitySession, requestedAccountId: string | null) {
      ...
      this.noticeKey =
        requestedAccountId && requestedAccountId !== session.currentAccount.id
          ? 'identity.account.fallbackNotice'
          : null;
    ...
    async refreshSession(): Promise<DemoIdentitySession | undefined> {
      if (this.renewing) {
        return undefined;
      }
      ...
      try {
        const session = await fetchIdentitySession({
          selectedAccountId: this.selectedAccountId ?? readStoredAccountId(),
        });

        this.applySession(session, this.selectedAccountId);
        this.initialized = true;
    ...
    async getCurrentOrRefreshAccessToken(): Promise<string | null> {
      if (this.accessToken && !isTokenExpired(this.expiresAt)) {
        return this.accessToken;
      }

      const storedToken = readStoredIdentityAccessToken();
...
export async function getCurrentOrRefreshIdentityAccessToken(): Promise<string | null> {
  try {
    return await useIdentitySessionStore().getCurrentOrRefreshAccessToken();
  } catch {
