...
describe('identity service API', () => {
  ...
  it('creates a session from selected account and token response', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(
        jsonResponse({
          accounts: [createAccount('user-default'), createAccount('user-selected')],
    ...
    await expect(fetchIdentitySession({ selectedAccountId: 'user-selected' })).resolves.toMatchObject({
